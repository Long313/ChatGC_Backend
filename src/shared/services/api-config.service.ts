...
export class ApiConfigService {
  ...
  get databaseConfig(): TypeOrmModuleOptions {
    ...
    return {
      entities,
      type: 'mongodb',
      url: `mongodb://${this.getString('DB_USERNAME')}:${this.getString(
        'DB_PASSWORD'
      )}@${this.getString('DB_HOST')}:${this.getString(
        'DB_PORT'
      )}/chatgc?authSource=admin&replicaSet=dbrs`,
  ...
  get mongoConfig() {
    return this.isDevelopment
      ? {
          uri: `mongodb://${this.getString('DB_HOST')}:${this.getString(
            'DB_PORT'
          )}/chatgc?retryWrites=false
      `,
          useNewUrlParser: true,
          useUnifiedTopology: true,
        }
      : {
          uri: `mongodb://${this.getString('DB_USERNAME')}:${this.getString(
            'DB_PASSWORD'
          )}@${this.getString('DB_HOST')}:${this.getString(
            'DB_PORT'
          )}/chatgc?authSource=admin&retryWrites=false
      `,
          useNewUrlParser: true,
          useUnifiedTopology: true,
        };
